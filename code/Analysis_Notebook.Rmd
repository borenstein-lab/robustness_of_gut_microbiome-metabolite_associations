...
```{r}
PATH_PROCESSED_DATA <- '../data/Processed_Data_Backup_29092020.RData'
PATH_METAB_PREDICT_RESULTS <- '../data/Combined_Metabolome_Predictability_Results.tsv'
PATH_REM_RESULTS <- '../data/REM_Results.tsv'
PATH_REM_MODELS <- '../data/Full_REM_Results.RData'
PATH_REM_RESULTS_SHUF <- '../data/REM_Results_Shuffled.tsv'
PATH_MELONNPAN_RESULTS <- '../data/MelonnPan_supp_table_3_withHMDB.xlsx'
PATH_ZIERER_METAB_MAP <- '../data/Zierer_et_al_2018/Metabolite_HMDB_Mappings.tsv'
PATH_ZIERER_RESULTS <- '../data/Zierer_et_al_2018/Variance components and phenotype associations of fecal metabolites.xlsx'
PATH_REM_SUBGROUPS <- '../data/Subgroup_REM_Results.tsv'
PATH_REM_SUBGROUP_MODELS <- '../data/Full_Subgroup_REM_Results.RData'
PATH_PERM_FEATURE_IMP <- '../data/Permutation_Feature_Importances.tsv'
PATH_PAIRWISE_CROSS_PRED <- '../data/Pairwise_Cross_Pred_Results.tsv'
PATH_PAIRWISE_CONTRIBS <- '../data/Pairwise_Contrib_Results.tsv'
PATH_PAIRWISE_CROSS_PRED_D <- '../data/Pairwise_Cross_Pred_Results_Healthy_Disease.tsv'
```
...
metab.pred.results <- read_delim(PATH_METAB_PREDICT_RESULTS, 
...
TODO: save to a file to avoid re-computing each notebook run.
...
if(!exists("REM.results")) {
  REM.results <- read.delim(PATH_REM_RESULTS, stringsAsFactors = F)
  load(PATH_REM_MODELS)
}
...
if(!exists("REM.results.shuffled")) {
  REM.results.shuffled <- read.delim(PATH_REM_RESULTS_SHUF, stringsAsFactors = F)
}
...
melonnpan.well.predicted <- read.xls(PATH_MELONNPAN_RESULTS, sheet=1, skip=1, header=TRUE)
...
twinsUK.metab.mappings <- read_delim(PATH_ZIERER_METAB_MAP, "\t", escape_double = FALSE, trim_ws = TRUE)
twinsUK.MM.var.explained <- read.xls(PATH_ZIERER_RESULTS, sheet=1, skip=1, header=TRUE)
...
if(!exists("REM.subgroup.results")) {
  REM.subgroup.results <- read.delim(PATH_REM_SUBGROUPS, stringsAsFactors = F)
  load(PATH_REM_SUBGROUP_MODELS)
}
...
*Altmann, André, et al. "Permutation importance: a corrected feature importance measure." Bioinformatics 26.10 (2010): 1340-1347.*   
...
```{r message = FALSE}
metab.contributors.Perm <- read_delim(PATH_PERM_FEATURE_IMP, 
    "\t", escape_double = FALSE, trim_ws = TRUE)
...
pairwise.comp.cross.pred <- read_delim(PATH_PAIRWISE_CROSS_PRED, 
    "\t", escape_double = FALSE, trim_ws = TRUE)

# (Without downsampling)
pairwise.comp.contribs <- read_delim(PATH_PAIRWISE_CONTRIBS, 
    "\t", escape_double = FALSE, trim_ws = TRUE)
...
for(tmp.metab.name in levels(feat.importance.stats$Compound.Name)) { # tmp.metab.name = "Cholic acid"
  ...
  # Add a label for the CP plots (CP = Cross-predictability)
  # We add a newline for cases where text won't fit in square
  tmp <- tmp %>%
    mutate(p.label.cp = ifelse(is.na(New.Spearman.rho.cp2), 
                               "", 
                               round(New.Spearman.rho.cp2,2))) %>%
    mutate(p.label.cp = ifelse(Significance.cp2 == "",
                               p.label.cp,
                               paste(p.label.cp, Significance.cp2))) %>%
    rowwise() %>% 
    mutate(p.label.cp = ifelse(n_distinct(tmp$Dataset1)>3,
                               gsub(" ","\n",p.label.cp),
                               p.label.cp))
    
  p3 <- ggplot(tmp, aes(x = Dataset1, y = Dataset2, fill = Well.Transferred)) + 
    geom_tile(color = "black") + 
    geom_text(aes(label=p.label.cp), 
              size = ifelse(n_distinct(tmp$Dataset1)>5,
                            2.3,
                            ifelse(n_distinct(tmp$Dataset1)>4,2.4,2.5))) +
...
```{r fig.width=13, fig.height=17, eval=F}
do.call("grid.arrange", c(all.cp.plots, ncol=6))
```

Specific plots for manuscript:

```{r fig.width=1.9, fig.height=2.2}
p <- all.cp.plots$`beta-Alanine`
print(p)

p <- all.cp.plots$`Sebacic acid`
print(p)

p <- all.cp.plots$`L-Tyrosine`
print(p)

p <- all.cp.plots$Taurine
print(p)

p <- all.cp.plots$`N-Acetylputrescine`
print(p)

p <- all.cp.plots$Xanthine
print(p)

p <- all.cp.plots$`Cholic acid`
print(p)
```
...
pairwise.cp.h.vs.d.raw <- read_delim(PATH_PAIRWISE_CROSS_PRED_D, 
    "\t", escape_double = FALSE, trim_ws = TRUE)
