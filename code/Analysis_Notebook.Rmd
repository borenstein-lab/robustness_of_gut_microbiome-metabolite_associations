...
print(paste0("Using a cutoff of FDR < ",
             ...
             rho.threshold.REM,", ", 
             nrow(REM.universal), 
             " metabolites are universally predictable by the microbiome."))
print(paste("Of these,", 
...
main.val.table <- REM.results %>% 
  ...
  mutate(Category = ifelse(HMDB %in% unique(REM.universal$HMDB),
                           "Universally well-predicted",
                           "Non-universally well-predicted")) %>%
...
ggplot(tmp, aes(x = Super.Class2, y = prop, fill = Category)) +
  ...
  ggtitle("Comparison of metabolite super-classes: universally well-predicted vs. others") +
  theme_classic() + 
...
We load a table from the paper supplementary that details for each metabolite how much of it's variation is explained by the microbiome.  
See: *Zierer, Jonas, et al. "The fecal metabolome as a functional readout of the gut microbiome." Nature genetics 50.6 (2018): 790-795.*

```{r TWINSUK_VALIDATION_VAR_EXPLAINED, fig.height=2.5, fig.width=5.1}
load("../data/Zierer_et_al_2018/Zierer_TwinsUK_Associations.RData")
...
twinsUK.MM.var.explained <- twinsUK.MM.var.explained %>%
  ...
  # We use the mapping to HMDB's already conducted for previous file
  left_join(twinsUK.associations %>% 
...
tmp <- REM.results %>%
  ...
  mutate(Universal = ifelse(HMDB %in% REM.universal$HMDB, "Universally\nwell-predicted", "Non-universally\nwell-predicted"))
...
ggplot(tmp, aes(x=Universal, fill = Universal, 
  ...
  xlab(NULL) + #xlab("Universally well-predicted\nby the microbiome") +
